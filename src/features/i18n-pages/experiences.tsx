...
export async function renderExperiencesPage(locale: SiteLocale = DEFAULT_SITE_LOCALE) {
  ...
  const publishedExperiences = await fetchPublishedExperiences(locale);
  const localizedCollectionFeatures =
      ...
      <section className="relative flex min-h-[72vh] items-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-20 pt-36 sm:px-10 lg:px-16">
          <p className="mb-6 font-body text-[0.6rem] uppercase tracking-[0.32em] text-white/32">
            {dictionary.experiences.threeWays}
          </p>
          <h1
          ...
          <p className="mt-8 max-w-2xl font-body text-sm leading-relaxed text-white/58 sm:text-[0.95rem]">
            {locale === DEFAULT_SITE_LOCALE
              ? 'CREARE approaches experience through three distinct paths: composed cultural encounters, commissions shaped around the guest, and discreet access reserved for the right context.'
          ...
          {locale === DEFAULT_SITE_LOCALE ? (
            <div className="space-y-5">
              ...
              <p className="font-body text-sm leading-relaxed text-white/40">
                It helps guests understand whether they are entering a finished encounter, shaping
                one around themselves, or moving within a more private tier of access.
              </p>
            </div>
          ) : null}
        </div>
          ...
          {categoryFeatures.map((feature) => (
            <article key={feature.label} className="flex flex-col">
              <Link
                ...
                aria-label={`${dictionary.common.enter} ${feature.label}`}
              >
