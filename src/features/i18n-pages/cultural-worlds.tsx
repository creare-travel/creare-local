      ...
      <section className="relative flex min-h-[72vh] items-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-20 pt-36 sm:px-10 lg:px-16">
          <p className="mb-6 font-body text-[0.6rem] uppercase tracking-[0.32em] text-white/32">
            {dictionary.culturalWorlds.atlasTitle}
          </p>
          <h1
          ...
          <p className="mt-8 max-w-2xl font-body text-sm leading-relaxed text-white/58 sm:text-[0.95rem]">
            {locale === DEFAULT_SITE_LOCALE
              ? 'A geography of layered places, read through memory, custodianship, and the conditions that make real cultural access meaningful. CREARE approaches place not as a destination list, but as a cultural world: a living continuity of geography, craft, ritual, local knowledge, and social memory that shapes how access can be offered and understood.'
              : dictionary.culturalWorlds.geography}
          </p>
        </div>
      </section>
        ...
        {destinations.length > 0 ? (
              ...
              heading={
                <>
                  <p className="mb-3 font-body text-[0.6rem] uppercase tracking-[0.28em] text-white/30">
                    {dictionary.culturalWorlds.explore}
                  </p>
            ...
            >
                  ...
                  <div className="relative flex aspect-[5/6] flex-col justify-between overflow-hidden border border-white/10 bg-white/[0.02] p-8 pb-10">
                    <div>
                      <p className="mb-6 font-body text-[0.6rem] uppercase tracking-[0.28em] text-white/32">
                        {dictionary.culturalWorlds.atlas}
                      </p>
                      <h3
                      ...
                      <p className="mt-8 max-w-[26ch] font-body text-sm leading-relaxed text-white/58">
                        A Cultural World is not a destination category, but a deeper reading of
                        place. It gathers geography, memory, local knowledge, and the forms of
                        access that make a place legible beyond arrival. Each world is approached as
                        a living cultural system, shaped by custodianship, continuity, and the
                        conditions that allow encounter to become meaningful. Istanbul, Bodrum, and
                        Cappadocia differ in terrain, tempo, and cultural expression, yet each
                        becomes clearer when read through the people, practices, and inherited
                        knowledge that sustain life there beyond sightseeing.
                      </p>
                    </div>
                  </div>
                </article>
              ...
              {destinations.map((destination, index) => {
                const href = destination.slug
                  ? localizePathname(`/cultural-worlds/${destination.slug}`, locale)
                  : null;
                      ...
                      : destination.slug === 'cappadocia'
                        ...
                        : destination.slug === 'bodrum'
                          ...
                          : (destination.highlight ??
                            destination.short_description ??
                            'Private cultural access composed around place, narrative, and depth.')
                    : (destination.highlight ?? destination.short_description ?? '');
          ...
          <div className="py-12">
            <p className="text-white/40 font-body text-sm leading-relaxed max-w-md">
              {dictionary.culturalWorlds.emptyState}
            </p>
          </div>
        )}
      </section>
